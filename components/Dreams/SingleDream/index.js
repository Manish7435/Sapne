...
const SingleDream = ({endPoint}) => {
  ...
  return (
    <>
          <div className=' sm:w-full w-auto overflow-y-scroll flex flex-col border-r-2 border-l-2 border-[#4673CA] sm:px-20 p-4 sm:pb-20 '>
          {loading ? <CardSkeleton/>: dreams.length ? dreams.map((dream)=>{
            return(
              <div key = {dream._id} className='bg-[#373F5B] mt-20 rounded-lg p-4'>
                ...
                <div><Image className='mt-2' src='/Redheart.svg' alt='like' height={20} width={20}/></div>
              </div>
            )
